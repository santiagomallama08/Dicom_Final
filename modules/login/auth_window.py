...
class AuthWindow:
    def __init__(self, root, on_success):
        ...
        img = Image.open("D:/Universidad/Software Dicom/Dicom_Final/imagenes/ucclogo.jpg")  # La ruta de la imagen que ya tienes
        img = img.resize((100, 100), Image.Resampling.LANCZOS)  # Ajustar tamaño de la imagen
        img_tk = ImageTk.PhotoImage(img)

        # Canvas para mostrar la imagen
        canvas = tk.Canvas(self.frame, width=120, height=120, bg="white", highlightthickness=0)
        ...
        self.tabs.grid(row=1, column=0, columnspan=2, pady=10)  # Usamos grid() para una alineación más controlada
        self.btn_login_tab = tk.Button(self.tabs, text="INICIAR SESIÓN", command=self.show_login, width=15, bg="#4CAF50", fg="white", font=("Arial", 12, "bold"))
        self.btn_login_tab.grid(row=0, column=0, padx=10)

        self.btn_registro_tab = tk.Button(self.tabs, text="REGISTRARSE", command=self.show_register, width=15, bg="#2196F3", fg="white", font=("Arial", 12, "bold"))
    ...
    def show_login(self):
        ...
        # Icono usuario
        tk.Label(self.content, text="Correo  👤", font=("Arial", 12), bg="white").grid(row=0, column=0, padx=5, pady=10, sticky="w")
        email = tk.Entry(self.content, width=30)
        email.grid(row=0, column=1, padx=5, pady=10)

        # Contraseña
        tk.Label(self.content, text="Contraseña  🛡️", font=("Arial", 11), bg="white").grid(row=1, column=0, padx=5, pady=10, sticky="w")
        ...
        tk.Button(btn_frame, text="INICIAR SESIÓN", width=15, command=do_login, bg="#4CAF50", fg="white", font=("Arial", 12, "bold")).pack()

    def show_register(self):
        self.clear_content()

        fields = [("👤 Nombre completo", "Nombre completo"),
        ...
        for idx, (icon, label) in enumerate(fields):
            tk.Label(self.content, text=icon, font=("Arial", 12), bg="white").grid(row=idx, column=0, padx=5, pady=10, sticky="w")
            ent = tk.Entry(self.content, width=30, show="*" if "Contraseña" in label else None)
        ...
        tk.Button(btn_frame, text="REGISTRARSE", width=15, command=do_register, bg="#2196F3", fg="white", font=("Arial", 12, "bold")).pack()
